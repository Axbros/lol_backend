...
graph:
	...
	@cp -f cmd/lol/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=lol lol
	@rm -f main.go lol.gv
...
.PHONY: build
# Build lol for linux amd64 binary
build:
	@echo "building 'lol', linux binary file will output to 'cmd/lol'"
	@cd cmd/lol && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
