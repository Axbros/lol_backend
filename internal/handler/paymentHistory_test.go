...
func newPaymentHistoryHandler() *gotest.Handler {
	...
	d.IDao = dao.NewPaymentHistoryDao(d.DB, c.ICache.(cache.PaymentHistoryCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &paymentHistoryHandler{iDao: d.IDao.(dao.PaymentHistoryDao)}
	iHandler := h.IHandler.(PaymentHistoryHandler)
...
func Test_paymentHistoryHandler_Create(t *testing.T) {
	h := newPaymentHistoryHandler()
	defer h.Close()
	testData := &types.CreatePaymentHistoryRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.PaymentHistory))
...
func Test_paymentHistoryHandler_DeleteByID(t *testing.T) {
	h := newPaymentHistoryHandler()
	defer h.Close()
	testData := h.TestData.(*model.PaymentHistory)
...
func Test_paymentHistoryHandler_UpdateByID(t *testing.T) {
	h := newPaymentHistoryHandler()
	defer h.Close()
	testData := &types.UpdatePaymentHistoryByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.PaymentHistory))
...
func Test_paymentHistoryHandler_GetByID(t *testing.T) {
	h := newPaymentHistoryHandler()
	defer h.Close()
	testData := h.TestData.(*model.PaymentHistory)
...
func Test_paymentHistoryHandler_List(t *testing.T) {
	h := newPaymentHistoryHandler()
	defer h.Close()
	testData := h.TestData.(*model.PaymentHistory)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListPaymentHistorysRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewPaymentHistoryHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewPaymentHistoryHandler()
}
