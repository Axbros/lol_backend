...
func newSmsHistoryHandler() *gotest.Handler {
	...
	d.IDao = dao.NewSmsHistoryDao(d.DB, c.ICache.(cache.SmsHistoryCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &smsHistoryHandler{iDao: d.IDao.(dao.SmsHistoryDao)}
	iHandler := h.IHandler.(SmsHistoryHandler)
...
func Test_smsHistoryHandler_Create(t *testing.T) {
	h := newSmsHistoryHandler()
	defer h.Close()
	testData := &types.CreateSmsHistoryRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.SmsHistory))
...
func Test_smsHistoryHandler_DeleteByID(t *testing.T) {
	h := newSmsHistoryHandler()
	defer h.Close()
	testData := h.TestData.(*model.SmsHistory)
...
func Test_smsHistoryHandler_UpdateByID(t *testing.T) {
	h := newSmsHistoryHandler()
	defer h.Close()
	testData := &types.UpdateSmsHistoryByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.SmsHistory))
...
func Test_smsHistoryHandler_GetByID(t *testing.T) {
	h := newSmsHistoryHandler()
	defer h.Close()
	testData := h.TestData.(*model.SmsHistory)
...
func Test_smsHistoryHandler_List(t *testing.T) {
	h := newSmsHistoryHandler()
	defer h.Close()
	testData := h.TestData.(*model.SmsHistory)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListSmsHistorysRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewSmsHistoryHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewSmsHistoryHandler()
}
