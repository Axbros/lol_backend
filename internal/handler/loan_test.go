...
func newLoanHandler() *gotest.Handler {
	testData := &model.Loan{}
	testData.ID = 1
	...
	d.IDao = dao.NewLoanDao(d.DB, c.ICache.(cache.LoanCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &loanHandler{iDao: d.IDao.(dao.LoanDao)}
	iHandler := h.IHandler.(LoanHandler)
...
func Test_loanHandler_Create(t *testing.T) {
	h := newLoanHandler()
	defer h.Close()
	testData := &types.CreateLoanRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Loan))
...
func Test_loanHandler_DeleteByID(t *testing.T) {
	h := newLoanHandler()
	defer h.Close()
	testData := h.TestData.(*model.Loan)
...
func Test_loanHandler_UpdateByID(t *testing.T) {
	h := newLoanHandler()
	defer h.Close()
	testData := &types.UpdateLoanByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Loan))
...
func Test_loanHandler_GetByID(t *testing.T) {
	h := newLoanHandler()
	defer h.Close()
	testData := h.TestData.(*model.Loan)
...
func Test_loanHandler_List(t *testing.T) {
	h := newLoanHandler()
	defer h.Close()
	testData := h.TestData.(*model.Loan)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListLoansRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewLoanHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewLoanHandler()
