...
func newResultHandler() *gotest.Handler {
	...
	d.IDao = dao.NewResultDao(d.DB, c.ICache.(cache.ResultCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &resultHandler{iDao: d.IDao.(dao.ResultDao)}
	iHandler := h.IHandler.(ResultHandler)
...
func Test_resultHandler_Create(t *testing.T) {
	h := newResultHandler()
	defer h.Close()
	testData := &types.CreateResultRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Result))
...
func Test_resultHandler_DeleteByID(t *testing.T) {
	h := newResultHandler()
	defer h.Close()
	testData := h.TestData.(*model.Result)
...
func Test_resultHandler_UpdateByID(t *testing.T) {
	h := newResultHandler()
	defer h.Close()
	testData := &types.UpdateResultByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Result))
...
func Test_resultHandler_GetByID(t *testing.T) {
	h := newResultHandler()
	defer h.Close()
	testData := h.TestData.(*model.Result)
...
func Test_resultHandler_List(t *testing.T) {
	h := newResultHandler()
	defer h.Close()
	testData := h.TestData.(*model.Result)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListResultsRequest{query.Params{
		Page:  0,
		Limit: 10,
